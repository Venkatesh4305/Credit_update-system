...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['employee-id']) && isset($_POST['password'])) {
        ...
        if (empty($employee_id) || empty($pass)) {
            ...
            // Check for the fixed admin credentials
            if ($employee_id === '  ' && $pass === 'password123') {
                // Admin login is successful, create session and redirect
                ...
                if ($result->num_rows > 0) {
                    ...
                    if ($pass === $userDetails['password']) {
                        // Password is correct, create session and redirect to teacher dashboard
        ...
        <div class="login-box">
            ...
            <form action="adminlogin.php" method="POST" class="login-form">
                <div class="form-group">
                    <label for="employee-id">Admin-ID</label>
                ...
                <button type="button" class="admin-login-btn" onclick="window.location.href='login.php';">Teacher Login</button>
            </form>
        </div>
    </div>
